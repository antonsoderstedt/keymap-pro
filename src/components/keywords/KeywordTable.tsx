...
interface Props {
  items: UniverseKeyword[];
  limit?: number;
}

/**
 * Konsoliderad sökordstabell – används av Sökord & innehåll-vyn.
 * Visar SERP-features och top-domäner (rikare version).
 */
export function KeywordTable({ items, limit = 500 }: Props) {
  if (items.length === 0) {
    return (
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Top-domäner</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {items.slice(0, limit).map((k, i) => (
              <TableRow key={i} className={k.isNegative ? "opacity-60" : ""}>
                <TableCell className="font-mono text-sm">
                  ...
                  {k.competitorGap && (
                    <Badge variant="outline" className="ml-2 border-warning/50 text-[10px] text-warning">
                      Gap
                    </Badge>
                  )}
                </TableCell>
                <TableCell className="text-right font-mono">
                  {k.searchVolume?.toLocaleString("sv-SE") ?? "—"}
                </TableCell>
                <TableCell className="text-right font-mono">
                ...
                <TableCell>
                  <Badge variant="outline">{DIMENSION_LABELS[k.dimension] || k.dimension}</Badge>
                </TableCell>
                <TableCell>
                  <Badge variant="secondary">{INTENT_LABELS[k.intent] || k.intent}</Badge>
                </TableCell>
                <TableCell className="text-xs text-muted-foreground">{k.funnelStage}</TableCell>
                <TableCell>
                  <Badge variant={PRIORITY_VARIANT[k.priority] || "outline"}>{k.priority}</Badge>
                </TableCell>
                ...
                <TableCell
                  ...
                  title={(k.serpFeatures || []).join(", ")}
                >
                  {(k.serpFeatures || []).slice(0, 2).join(", ") || "—"}
                </TableCell>
                <TableCell
                  className="max-w-[180px] truncate text-[11px] text-muted-foreground"
                  title={(k.topRankingDomains || []).join(", ")}
                >
                  {(k.topRankingDomains || []).slice(0, 2).join(", ") || "—"}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
