...
serve(async (req) => {
  ...
  try {
    ...
    const clusters: ClusterSummary[] = Array.from(clusterMap.entries()).map(([name, kws]) => {
      ...
      const bestPos =
        gscForCluster.length > 0
          ? Math.min(...gscForCluster.map((r) => r.position))
          : null;
      ...
      return {
        ...
        avg_kd:
          kdVals.length > 0
            ? kdVals.reduce((s: number, k: any) => s + (k.kd ?? 0), 0) / kdVals.length
            : null,
        avg_cpc:
          cpcVals.length > 0
            ? cpcVals.reduce((s: number, k: any) => s + (k.cpc ?? 0), 0) / cpcVals.length
            : null,
    ...
    const snapshot: SeoContentSnapshot = {
      ...
      content_briefs: (briefs ?? []).map((b: any) => ({
        cluster: b.cluster,
        exists: true,
        payload: null,
      })),
